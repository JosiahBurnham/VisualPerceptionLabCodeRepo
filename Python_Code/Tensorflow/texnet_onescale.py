...
classify artificial textures into 15 categories

Training set naming conventions
-------------------------------
train_set_text_onescale_{init}_{setnum}

    init   : 3 character initial string (idl = ideal)
    setnum : 1,2,3...

Requires
--------
filter_mat_onescale.mat : This program contains a set of 16x16 filters which
comprise the input layer to the convolutional neural network

Usage
-----
To load the module : "import texnet_onescale as tx"


TO DO
-----
     - Perhaps penalize the biases (l2)
     - Add V2 + V4 sparsity L1 constraints
...
###############################################################################
num_files       = 6  # 6 files, 5 of which we train on, 1 of which we exclude
num_cycles      = 10  # times to train on each set
num_input       = 64*64
num_classes     = 15
train_iters     = 50
batch_size      = 100
...
def init_weights(filter_str,num_v2,num_v4):
    ...
    weights = {
                ...
                # (downsample from 64x64 to 8x8 + then apply 3x3 filters
                # 4x4   convolutional filters, 6 input, num_v2 outputs
                'wc2': tf.Variable(0.1*tf.random_normal([3,3,6,num_v2]),dtype=tf.float32),
                # Fully connected: 2 outputs (L/R)
                'wd1': tf.Variable(0.1*tf.random_normal([3*3*num_v2,num_classes]),dtype=tf.float32) #,
             #   'wd2': tf.Variable(0.1*tf.random.normal([num_v4,num_classes]),dtype=tf.float32)
            }
    
    biases  = {
                # 6 filters
                'bc1': tf.Variable(tf.zeros([6]),dtype=tf.float32, trainable = False),        
                # num_v2 filters
                'bc2': tf.Variable(tf.zeros([num_v2]),dtype=tf.float32, trainable = True),
                # 2 units
                'bd1': tf.Variable(tf.zeros([num_classes]),dtype=tf.float32,trainable = True) #,
...
def train_model(fname="train_set_text_onescale_idl", fit_ideal = 1,  num_v2 = 6, num_v4 = 6, testfile = 5, l2penalty = 0.0, sparse_penalty = 0.0):
    ...
    matfile_name = "./MODELS/" + "texnet_onescale_" + fname + "_" + str(num_v2) + "_" + str(num_v4) + "_" + str(testfile)+ "_" + str(l2penalty) + "_" +    str(sparse_penalty)
    ...
    with tf.Session() as sess:
        ...
        if fit_ideal:
            ...
            test_batch_all_y = scipy.io.loadmat(fname_test)['R']
        
        # test batch for evaluating accurcay
        test_batch_x = test_batch_all_x[0:500,:] 
        test_batch_y = test_batch_all_y[0:500,:]    
        
        # second test batch for hyper-parameter optimization
        test_batch_x_hp = test_batch_all_x[500:1000,:]
        test_batch_y_hp = test_batch_all_y[500:1000,:]
        ...
        for cur_cycle in range(num_cycles):
            for cur_trainset in train_list:
                ...
                if fit_ideal:
                    ...
                    train_batch_all_y = scipy.io.loadmat(fname_train)['R']   
...
def train_model_batch(fname="train_set_text_onescale_idl", fit_ideal = 1,  num_v2 = 6, num_v4 = 6, test_file=5, l2penalty_vec=[0.05], sparse_penalty_vec =[0.05]):
            
    for i in range(len(l2penalty_vec)):
        for j in range(len(sparse_penalty_vec)):
            train_model(fname,fit_ideal,num_v2,num_v4,test_file,l2penalty_vec[i],sparse_penalty_vec[j])
