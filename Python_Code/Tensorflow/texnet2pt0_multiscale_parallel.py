...
# NORMALIZATION/DROPOUT CONSTANTS
prob_dropout 	= 0.5
...
def conv_net(x,fbank,weights,biases):
    ...
    conv1       = tf.nn.local_response_normalization(conv1,depth_radius=5,bias=1,alpha=1e-04,beta=0.5)    
    ...
    conv2       = tf.nn.local_response_normalization(conv2,depth_radius=5,bias=1,alpha=1e-04,beta=0.5)   

    conv2       = tf.nn.dropout(conv2,prob_dropout)
