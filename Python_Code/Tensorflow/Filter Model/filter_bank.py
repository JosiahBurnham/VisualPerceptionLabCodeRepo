...
class FilterBank(keras.layers.Layer):
    ...
    def _filter_images(self, input_image):
        ...
        for i in range(len(self.filter_bank)):
            ...
            cur_filter = tf.Variable(cur_filter, dtype=tf.float32, trainable=False)

            filtered_image = self._convolution(input_image, cur_filter)

            filtered_images = np.append(filtered_images, filtered_image, axis=3)
    ...
    def _load_filters(self):
        ...
        for i in range(len(self.filters)):
            loaded_filters = scipy.io.loadmat(self.filters[i][0])[self.filters[i][1]]
    ...
    def _convolution(input_image, cur_filter):
        ...
        """
        input_data = tf.nn.conv2d(input_image, cur_filter, strides=[1, 1, 1, 1],padding="SAME")
