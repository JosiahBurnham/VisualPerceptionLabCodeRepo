...
Usage
-----
To load the module : "import texnet1_multiscale_small as txms1"
...
###############################################################################
def init_weights(filter_str,num_v2,num_v4,num_classes):
    global weights
    global biases
    ...
    weights = {
                ...
                'wc2': tf.Variable(0.1*tf.random_normal([v4_stride,v4_stride,nfixed_filters,num_v2]),dtype=tf.float32),
                # Fully connected: 2 outputs (L/R)
                'wd1': tf.Variable(0.1*tf.random_normal([v4_stride*v4_stride*num_v2,num_v4]),dtype=tf.float32),
                'wd2': tf.Variable(0.1*tf.random_normal([num_v4,num_classes]),dtype=tf.float32)
            }
...
def train_model(fname="train_set_text_onescale_idl", num_files = 6, num_v2 = 6, num_v4 = 6, testfile = 5, l2penalty = 0.0, sparse_penalty = 0.0):
    
    # Initialize network weights: Global varibles <weights>, <biases>
    filter_str = 'filter_mat_onescale.mat'     
    init_weights(filter_str,num_v2,num_v4,num_classes)
